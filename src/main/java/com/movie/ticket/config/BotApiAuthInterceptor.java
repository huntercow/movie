...
@Component
public class BotApiAuthInterceptor implements HandlerInterceptor {

    private final ObjectMapper objectMapper;
    private final AgentService agentService;

    public BotApiAuthInterceptor(
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        String actualToken = request.getHeader("X-Bot-Token");
        String installationId = request.getHeader("X-Bot-Installation-Id");
        if (StringUtils.hasText(actualToken) && StringUtils.hasText(installationId)) {
            try {
                AgentAccess access = agentService.authenticateRuntime(
                        actualToken,
                        AgentType.WECHAT_BOT,
                        installationId
                );
        ...
        writeError(response, HttpServletResponse.SC_UNAUTHORIZED, "invalid or unactivated bot token");
        return false;
    }
