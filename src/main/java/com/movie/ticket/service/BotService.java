...
public class BotService {
    ...
    public BotQuoteResponse createQuoteFromImage(BotImageQuoteRequest request) {
        ...
        BotMessage existingMessage = botMessageRepository.findByWechatIdAndMessageId(request.wechatId(), request.messageId())
                .orElse(null);
    ...
    private Customer upsertCustomer(String wechatId, String nickname, String avatarUrl) {
        Customer customer = customerRepository.findByWechatId(wechatId).orElseGet(() -> {
    ...
    private PaymentRecord resolvePaymentRecord(BotCreateOrderRequest request, Customer customer) {
        if (StringUtils.hasText(request.paymentRecordNo())) {
            PaymentRecord paymentRecord = paymentRecordRepository.findByPaymentRecordNo(request.paymentRecordNo())
        ...
        return paymentRecordRepository.findFirstByQuoteNoAndCustomerNoAndStatusOrderByConfirmedAtDesc(
                        request.quoteNo(),
                        customer.getCustomerNo(),
