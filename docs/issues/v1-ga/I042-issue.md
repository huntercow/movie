...
blocked_by: [I038, I040, I041]
labels: [release, p0, extension, backend, breaking-contract, manual-acceptance]
github_issue: https://github.com/huntercow/movie/issues/85
---
...
交付“执行分批灰度与安全回退准备”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 发布/启用/观察/回退 runbook，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 逐账号确认，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 拒绝旧扩展和旧写合同，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
