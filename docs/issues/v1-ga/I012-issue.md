...
effort: L
parallelizable: true
blocked_by: [I009, I011]
labels: [feature, p1, web, extension]
github_issue: https://github.com/huntercow/movie/issues/25
---

# I012 实现扩展实时连接与控制台实时状态

## 业务价值

交付“实现扩展实时连接与控制台实时状态”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 扩展重连/能力上报/配置刷新，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When Web 实时订阅，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 断线显示更新中断而非伪造离线，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
