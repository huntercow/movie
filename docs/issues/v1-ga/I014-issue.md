...
blocked_by: [I013]
labels: [feature, p1]
github_issue: https://github.com/huntercow/movie/issues/29
---
...
交付“实现时区营业时间规则”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 每天最多两段，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 支持跨夜与时区，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 拒绝重叠，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 服从高优先级停止原因，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
