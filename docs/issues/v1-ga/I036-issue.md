...
effort: M
parallelizable: true
blocked_by: [I012, I021, I034]
labels: [feature, p1, extension, backend]
github_issue: https://github.com/huntercow/movie/issues/73
---

# I036 消费业务通知并验证扩展消息安全

## 业务价值

交付“消费业务通知并验证扩展消息安全”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 消费后端通知，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 浏览器通知去重并链接控制台，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 消息来源/schema/敏感字段校验，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
