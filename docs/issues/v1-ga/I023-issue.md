...
effort: M
parallelizable: true
blocked_by: [I019, I020, I022]
labels: [feature, p1, backend]
github_issue: https://github.com/huntercow/movie/issues/47
---
...
交付“建立经营统计投影与口径测试”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 已收款/完成/利润/自动完成率，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 账号和用户时区，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 排除未归属与未知利润，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
...
- Blocked by: I019, I020, I022
- Blocks: I027, I041, I044
- 可并行：是
