...
effort: L
parallelizable: true
blocked_by: [I015, I021, I023, I025, I047]
labels: [feature, p1, backend]
github_issue: https://github.com/huntercow/movie/issues/55
---
...
交付“实现今日运营收件箱”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。

## 范围

- 按就绪/恢复任务/订单任务/进行中/指标排序
- 危险动作进入详情
- 显示实时中断
...
- Given 前置合同和依赖满足，When 按就绪/恢复任务/订单任务/进行中/指标排序，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 危险动作进入详情，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 显示实时中断，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
...
- Blocked by: I015, I021, I023, I025, I047
- Blocks: 无
- 可并行：是
