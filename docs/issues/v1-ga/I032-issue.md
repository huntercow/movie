...
github_issue: https://github.com/huntercow/movie/issues/65
---
...
交付“实现良票账号与显式账号绑定”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 多账号加密凭据，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 闲鱼账号显式绑定，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When Accepted Order 固定引用，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 失效触发停止，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
