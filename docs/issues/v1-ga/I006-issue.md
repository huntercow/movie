...
blocked_by: [I005, I046]
labels: [feature, p0, backend, data-isolation]
github_issue: https://github.com/huntercow/movie/issues/17
---
...
交付“建立稳定闲鱼经营账号模型”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。

## 范围

- 以 userId + sellerUserId 唯一
- 账号承载历史/统计/配置
- 增加隔离与唯一性测试
...
- Given 前置合同和依赖满足，When 以 userId + sellerUserId 唯一，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 账号承载历史/统计/配置，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 增加隔离与唯一性测试，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
...
- Blocked by: I005, I046
- Blocks: I007, I008, I013, I019, I030, I031, I032
- 可并行：否
