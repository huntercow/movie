---
id: I020
title: "固化 Accepted Order 续跑与金额验证门禁"
status: proposed
epic: E03
milestone: V1-GA
priority: P0
...
github_issue: https://github.com/huntercow/movie/issues/41
---

# I020 固化 Accepted Order 续跑与金额验证门禁

## 业务价值

交付“固化 Accepted Order 续跑与金额验证门禁”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。

## 范围

- waiting-payment 持久化为切点
- 幂等续跑
- 金额不一致仅退款或关闭
...
- Given 前置合同和依赖满足，When waiting-payment 持久化为切点，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 幂等续跑，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 金额不一致仅退款或关闭，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
