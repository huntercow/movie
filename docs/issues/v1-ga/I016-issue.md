...
blocked_by: [I013]
labels: [feature, p1]
github_issue: https://github.com/huntercow/movie/issues/33
---
...
交付“实现停止时段一次性回复”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。

## 范围

- 持久化 Closed-Intake Period 幂等
- 新截图不 OCR/报价
- Accepted Order 会话继续回复
...
- Given 前置合同和依赖满足，When 持久化 Closed-Intake Period 幂等，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 新截图不 OCR/报价，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When Accepted Order 会话继续回复，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
