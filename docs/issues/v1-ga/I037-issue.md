...
blocked_by: [I028, I035, I036]
labels: [integration, p1, web, extension, database, breaking-contract]
github_issue: https://github.com/huntercow/movie/issues/75
---

# I037 下线旧手工入口并完成跨模块构建

## 业务价值

交付“下线旧手工入口并完成跨模块构建”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。

## 范围

- 迁移并删除 xianyu-manual
- 角色隔离命令入口
- 构建 Web/扩展并检查 Manifest
...
- Given 前置合同和依赖满足，When 迁移并删除 xianyu-manual，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 角色隔离命令入口，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 构建 Web/扩展并检查 Manifest，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
