...
github_issue: https://github.com/huntercow/movie/issues/61
---
...
交付“实现版本化可视化话术配置”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 可视化分类编辑，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 受控变量和预览，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 默认/账号覆盖，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 不可变发布版本，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
