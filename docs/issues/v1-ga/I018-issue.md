...
parallelizable: true
blocked_by: [I003]
labels: [discovery, p0, external-action]
github_issue: https://github.com/huntercow/movie/issues/37
---
...
交付“确认未知外部结果的对账证据”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。

## 范围

- timebox 2 天
- 确认上游查询和发货证据
- 冻结 Reconciliation 字段
- 不足则阻塞
...
- Given 前置合同和依赖满足，When 确认上游查询和发货证据，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 冻结 Reconciliation 字段，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 不足则阻塞，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
