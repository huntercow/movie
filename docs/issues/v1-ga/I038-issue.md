...
blocked_by: [I007, I037]
labels: [migration, p0, database, manual-acceptance]
github_issue: https://github.com/huntercow/movie/issues/77
---
...
交付“执行迁移与回退演练”能力，使业务用户或治理角色获得可验证结果，而不是仅完成技术资源修改。
...
- Given 前置合同和依赖满足，When 生产规模脱敏数据演练，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 核对隔离和统计，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
- Given 前置合同和依赖满足，When 回退 UI 不回滚迁移/证据/新写合同，Then 行为、持久状态与审计证据符合 PRD，错误路径明确失败。
