...
启动时 Flyway 自动依次执行全部版本迁移：

- `V1` 创建用户、授权、会话、Agent Token/安装、闲鱼账号、用户上游账号和审计表。
- `V2` 为已经存在的原业务表补充 `user_id`。
- `V3` 创建持久任务队列 `job_task`。
- `V4` 保存插件当前闲鱼账号快照，`V5` 将未设置有效期的旧 Agent Token 转为待审批。
- `V5.1` 在新库中幂等建立原业务表的基线；已有安装不会覆盖既有表，保证依赖这些表的后续迁移可顺序执行。
- `V6` 增加闲鱼实付金额核验字段，`V7` 增加唯一发货尝试字段，`V8` 隔离历史未知发货认领记录。
